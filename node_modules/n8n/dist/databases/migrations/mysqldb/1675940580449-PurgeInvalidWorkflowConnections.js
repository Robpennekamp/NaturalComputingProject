    ...
    async up({ queryRunner, tablePrefix, migrationName, logger }) {
        ...
        workflows.forEach(async (workflow) => {
            const connections = typeof workflow.connections === 'string'
                ? (0, n8n_workflow_1.jsonParse)(workflow.connections)
                : workflow.connections;
            const nodes = typeof workflow.nodes === 'string' ? (0, n8n_workflow_1.jsonParse)(workflow.nodes) : workflow.nodes;
            ...
            const [updateQuery, updateParams] = queryRunner.connection.driver.escapeQueryWithParameters(`UPDATE \`${tablePrefix}workflow_entity\`
					...
					WHERE id = '${workflow.id}'`, { connections: JSON.stringify(connections) }, {});
